...
class ASTGCNDataBuilder:
    ...
    def build_adjacency_matrix(self, stops_info, method='distance', threshold=5.0):
        ...
        # 연결성 확인
        connected_components = count_connected_components(adj_matrix)
    ...
    def save_for_mstgcn(self, graph_signal_matrix, adj_matrix, stops_info, time_range, 
        ...
        # 3. CSV 형식으로도 저장 (분석용) - 샘플만
        csv_file = os.path.join(output_dir, 'gapyeong_drt_sample.csv')
        self._save_sample_csv(graph_signal_matrix, stops_info, time_range, csv_file)
        
        logger.info(f"Data saved to {output_dir}")
        logger.info(f"- NPZ file: {data_file}")
        logger.info(f"- Metadata: {metadata_file}")
        logger.info(f"- Sample CSV: {csv_file}")
        
        return data_file, metadata_file, csv_file
    
    def _save_sample_csv(self, graph_signal_matrix, stops_info, time_range, csv_file, sample_size=1000):
        """샘플 시계열 데이터를 CSV로 저장"""
        data_list = []
        
        stop_ids = list(stops_info.keys())[:10]  # 처음 10개 정류장만
        sample_times = time_range[:100]  # 처음 100 시간만
        
        for t_idx, timestamp in enumerate(sample_times):
            for stop_id in stop_ids:
        ...
        logger.info(f"Sample CSV saved with {len(df)} records")
